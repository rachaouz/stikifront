...
def dashboard_home(request: Request, _: User = Depends(get_current_user)):
    ...
    high   = db.query(ScanHistory).filter(ScanHistory.final_verdict == "high").count()
    medium = db.query(ScanHistory).filter(ScanHistory.final_verdict == "medium").count()
    low    = db.query(ScanHistory).filter(ScanHistory.final_verdict == "low").count()
    by_type = db.query(ScanHistory.ioc_type, func.count(ScanHistory.id)).group_by(ScanHistory.ioc_type).all()

...
def get_cve_report(cve_id: str) -> dict:
    ...
    if not is_valid_cve(cve_id):
        return {"error": f"Format CVE invalide : {cve_id}"}
    ...
    # 3. Risk — stocké sous "risk" ET "global_risk_score" pour compatibilité
    risk = calculate_cve_risk(result.get("cvss_score"))
    result["risk"]             = risk          # ← utilisé par ioc_scan_service
    result["global_risk_score"] = risk
    result["final_verdict"]    = risk["level"] # ← utilisé par ioc_analysis

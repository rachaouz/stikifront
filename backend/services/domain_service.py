...
def get_creation_date(domain: str, vt_data: dict) -> str:
    ...
    try:
        ...
        if isinstance(cd, list):
            cd = cd[0]
        if cd:
            return cd.strftime("%Y-%m-%d") if hasattr(cd, 'strftime') else str(cd)[:10]
    except:
        pass
...
PHISHING_KEYWORDS_DOMAIN = [
    "secure", "verify", "alert", "update", "login", "confirm",
    "billing", "account", "recover", "locked", "suspended",
    "validate", "authenticate", "reset", "storage", "wallet"
]
IMPERSONATED_BRANDS_DOMAIN = [
    "paypal", "apple", "google", "amazon", "microsoft", "facebook",
    "netflix", "dropbox", "instagram", "linkedin", "coinbase", "binance"
]

def analyze_domain_phishing(domain: str) -> dict:
    domain_lower = domain.lower()
    signals = []

    # Marque imitée
    for brand in IMPERSONATED_BRANDS_DOMAIN:
        if brand in domain_lower and domain_lower != f"{brand}.com":
            signals.append(f"Imite marque: {brand}")
            break

    # Keywords phishing
    found_kw = [kw for kw in PHISHING_KEYWORDS_DOMAIN if kw in domain_lower]
    if len(found_kw) >= 2:
        signals.append(f"Keywords phishing: {', '.join(found_kw[:3])}")

    phishing_score = 0
    if any("Imite" in s for s in signals):
        phishing_score += 40
    if any("Keywords" in s for s in signals):
        phishing_score += 25

    return {
        "signals": signals,
        "score": min(100, phishing_score),
...
def get_domain_report(domain: str) -> dict:
    ...
    registrar = vt_data.get("registrar") or ""
    if not registrar or registrar.strip() == "":
     try:
         import whois
         w = whois.whois(domain)
    ...
    creation_date = get_creation_date(domain, vt_data)
    phishing = analyze_domain_phishing(domain)
    if phishing["is_suspicious"] and malicious == 0:
     global_score = max(global_score, phishing["score"])
     global_level = "High" if phishing["score"] >= 60 else "Medium"
     confidence   = "Moderate"
